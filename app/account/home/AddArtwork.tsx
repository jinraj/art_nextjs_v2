...
const artworkSchema = z.object({
  ...
  // we accept number but will append as string in FormData
  price: z.number({ required_error: "Price is required", invalid_type_error: "Price must be a number" }),
  artType: z.enum(["Paintings", "Photography", "Decors"], { required_error: "Select an artwork type" }),
  // react-hook-form provides FileList for file input; we validate length > 0
  images: z
    .any()
    .refine((files: FileList | File[] | null) => !!files && (Array.isArray(files) ? files.length > 0 : (files as FileList).length > 0), "At least one image is required"),
...
export default function AddArtwork() {
  ...
  const {
    ...
    watch,
    resetField,
  } = useForm<ArtworkFormInputs>({
    resolver: zodResolver(artworkSchema),
    defaultValues: { isHidden: false, isSold: false },
  });
